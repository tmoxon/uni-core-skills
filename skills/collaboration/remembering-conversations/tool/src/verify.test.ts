...
describe('verifyIndex', () => {
  ...
  const archiveDir = path.join(testDir, '.config', 'uni', 'conversation-archive');
  const dbPath = path.join(testDir, '.config', 'uni', 'conversation-index', 'db.sqlite');

  beforeEach(() => {
    // Create test directories
    fs.mkdirSync(path.join(testDir, '.config', 'uni', 'conversation-index'), { recursive: true });
